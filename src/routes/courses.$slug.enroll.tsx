...
import { toast } from "sonner";
import { useRef, useState } from "react";
import { Upload, Check, Clock3, ShieldCheck, Copy, PartyPopper, Lock, PlayCircle, Wrench, ArrowRight } from "lucide-react";
import { cn } from "@/lib/utils";
import { motion, AnimatePresence } from "motion/react";
...
function Enroll() {
  ...
  function getFreeAccess() {
    ...
    toast.success("You're in — enjoy the course!");
  }
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 sm:px-6 py-12 md:py-16">
      ...
      {isFree ? (
        <div className="mt-8">
          {!freeUnlocked ? (
            <div className="max-w-md mx-auto rounded-lg border border-border bg-surface p-6">
              <h3 className="font-display text-lg font-semibold">Get instant free access</h3>
              <p className="mt-2 text-sm text-muted-foreground">
                No payment needed — just tell us where to send access.
              </p>
              <div className="mt-5 space-y-4">
                <div>
                  <Label htmlFor="fname">Full name</Label>
                  <Input id="fname" value={name} onChange={(e) => setName(e.target.value)} placeholder="Rohan Malhotra" className="mt-1.5 bg-background" />
                </div>
                <div>
                  <Label htmlFor="femail">Email</Label>
                  <Input id="femail" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" className="mt-1.5 bg-background" />
                </div>
              </div>
              <Button size="lg" onClick={getFreeAccess} className="mt-6 w-full bg-primary text-primary-foreground hover:bg-primary/90 font-medium">
                Get Free Access <ArrowRight className="ml-1.5 size-4" />
              </Button>
            </div>
          ) : (
            <div>
              <div className="rounded-lg border border-primary/40 bg-surface p-10 text-center glow-primary">
                ...
                <h3 className="mt-5 font-display text-3xl font-bold">You're in.</h3>
                <p className="mt-3 text-muted-foreground max-w-md mx-auto">
                  Your access to <span className="text-foreground font-medium">{course.title}</span> is unlocked below.
                </p>
              </div>
              <div className="mt-8 rounded-lg border border-border bg-surface p-6">
                <div className="flex items-center justify-between">
