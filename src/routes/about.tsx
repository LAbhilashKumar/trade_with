...
import { createFileRoute } from "@tanstack/react-router";
import { ShieldCheck, Award, LineChart } from "lucide-react";
  ...
  return (
    <div>
      ...
      <section className="mx-auto max-w-4xl px-4 sm:px-6 py-20">
        ...
        <ol className="mt-10 space-y-6 border-l border-border pl-8 relative">
          {[
            { y: "2020", t: "First live account. Started trading forex full-time." },
            { y: "2021", t: "Documented the first written, tested edge." },
            { y: "2022", t: "Launched private mentorship. First students onboarded." },
            { y: "2024", t: "TWS opens to public cohorts." },
            { y: "2026", t: "2,400+ students trained to date." },
          ].map((m) => (
            <li key={m.y} className="relative">
              <span className="absolute -left-[38px] top-1 size-3 rounded-full bg-bull border-4 border-background" />
              <div className="font-mono text-sm text-bull">{m.y}</div>
